...
mod tests {
    ...
    fn multisig_deposit_is_taken_and_returned_when_updated() {
        new_test_ext().execute_with(|| {
            ...
            assert_ok!(MultiAccount::update(
                Origin::signed(multi_id),
                1,
                vec![1, 3]
            ));
            assert_eq!(
    ...
    #[test]
    fn other_signatories_can_approve_first() {
        new_test_ext().execute_with(|| {
            let multi_id = MultiAccount::multi_account_id(2);
    ...
    fn multiaccount_update_fails_if_has_active_multisigs() {
        new_test_ext().execute_with(|| {
            ...
            assert_noop!(
                MultiAccount::update(Origin::signed(multi_id), 1, vec![1, 3]),
                Error::<Test>::ActiveMultisigs
            );
    ...
    fn multiaccount_remove_fails_if_has_active_multisigs() {
        new_test_ext().execute_with(|| {
            ...
            assert_noop!(
                MultiAccount::remove(Origin::signed(multi_id)),
                Error::<Test>::ActiveMultisigs
            );
